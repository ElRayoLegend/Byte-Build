...
public class CarritoServlet extends HttpServlet {
    ...
    // Obtener la lista del carrito actual
    List<Carrito> carrito = carritoService.listarCarrito();
    
    // Crear una nueva factura
    Factura factura = new Factura();
    factura.setFechaFactura(new Date()); // Establecer la fecha actual
    
    // Crear una descripción concatenando los nombres de los productos
    StringBuilder descripcion = new StringBuilder();
    double totalFactura = 0;

    for (Carrito item : carrito) {
        // Agregar cada ítem del carrito a la descripción de la factura
        descripcion.append(item.getNombreProducto()).append(" ");
        totalFactura += item.getTotal();
    }

    factura.setDescripcion(descripcion.toString());
    factura.setMonto(totalFactura);
